...
function findPlayerById(lobby, playerId) {
    if (!lobby) return null;
    return lobby.players.find(p => p.player_id === playerId);
}
...
function chooseRandomHost(lobby) {
    if (!lobby) return null;
...
// ==================================================
// FIND NEXT HOST AFTER RECONNECT TIMEOUT
// ==================================================
...
function broadcastLobby(lobby, code) {
    ...
    for (const player of lobby.players) {
        sendToSocket(player.socket, message);
    }
}
...
function broadcastToOthers(lobby, senderSocket, data) {
    if (!lobby) return;
    ...
    for (const player of lobby.players) {
        if (
            ...
            sendToSocket(player.socket, data);
        }
    }
}
...
function removePlayerFromLobby(lobby, player) {
    if (!lobby || !player) return;

    if (player.disconnect_timer) {
        clearTimeout(player.disconnect_timer);
        player.disconnect_timer = null;
    }
...
function leaveLobby(socket) {
    ...
    const player =
        findPlayerBySocket(lobby, socket);
    ...
    // CHỈ CÒN 1 NGƯỜI?
    //
    // Người bấm Back được xóa.
    // Người còn lại vẫn tồn tại nhưng lobby
    // không bị xóa nếu vẫn còn người.
    // ==========================================
    ...
    if (wasHost) {
        ...
        const newHost =
            chooseRandomHost(lobby);

        if (!newHost) {
            console.log(
                "NO ONLINE PLAYER AVAILABLE FOR NEW HOST:",
                code
            );
        }
    }
    ...
    // ==========================================
    // CẬP NHẬT LOBBY CHO NGƯỜI CÒN LẠI
    // ==========================================
...
function schedulePlayerRemoval(lobby, player) {
    ...
    player.disconnect_timer = setTimeout(
        () => {
            const currentLobby =
                lobbies.get(lobby.code);
            ...
            if (
                ...
                currentPlayer.disconnect_timer = null;
                return;
            }
...
function reconnectLobby(
    ...
    socket.role = player.role;
    socket.isHost = player.is_host;
...
function changeRole(socket, data) {
    const lobby =
        lobbies.get(
            socket.lobbyCode
        );
...
function gameStatus(socket, data) {
    const code =
        socket.lobbyCode;
...
function playerState(socket, data) {
    const lobby =
        lobbies.get(
            socket.lobbyCode
        );
...
function readyChange(socket, data) {
    const code =
        socket.lobbyCode;
...
function kickPlayer(socket, data) {
    const code =
        socket.lobbyCode;
